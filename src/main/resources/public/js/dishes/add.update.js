layui.use(['form', 'layer', 'formSelects'], function () {
     ...
     * 表单Submit监听
     */
    form.on('submit(addOrUpdateDishes)', function (data) {
        ...
        // 请求的地址
        var url = ctx + "/dishes/add"; // 添加操作

        // 判断菜品ID是否为空，如果不为空则为更新操作
        if ($("[name='id']").val()) {
            // 更新操作
            var url = ctx + "/dishes/update";
        }
     ...
     */
    var dishesId = $("[name='id']").val();
    formSelects.config("selectDishesType",{
        type:"post", // 请求方式
        searchUrl: ctx+"/dishestype/queryAllDishesType?dishesId="+dishesId,  // 请求地址
